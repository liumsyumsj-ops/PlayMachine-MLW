...
# ============================================================
train_clean = pd.read_csv("train_processed.csv")
test_clean  = pd.read_csv("test_processed.csv")
train_ids   = pd.read_csv("spaceship-titanic/train.csv", usecols=['PassengerId'])
test_ids    = pd.read_csv("spaceship-titanic/test.csv",  usecols=['PassengerId'])
...
np.save("oof_rf3.npy",  oof_proba)
np.save("test_rf3.npy", test_proba)
...
im = ax.imshow(cm, cmap=matplotlib.colors.LinearSegmentedColormap.from_list('custom', ['#424d95', 'white', '#90b8f1']))
...
ax.plot(fpr, tpr, color='#424d95', lw=2, label=f'AUC = {val_auc:.4f}')
ax.plot([0,1],[0,1], color='#9dabd0', linestyle='--', lw=1, label='Random baseline')
...
ax.barh(top20.index, top20.values, color='#90b8f1', edgecolor='#6182cc', linewidth=0.5)
ax.set_title('Top 20 Feature Importances'); ax.set_xlabel('Importance')

plt.tight_layout()
plt.savefig("spaceship-titanic/rf3_evaluation.png", dpi=150, bbox_inches='tight')
plt.show()
print("\nSaved: rf3_evaluation.png")

fig, ax = plt.subplots(figsize=(7, 4))
bars = ax.bar([f'Fold {i+1}' for i in range(5)], cv_scores,
              color='#90b8f1', edgecolor='#6182cc', linewidth=0.8)
for bar, val in zip(bars, cv_scores):
    ax.text(bar.get_x()+bar.get_width()/2, bar.get_height()+0.002,
            f'{val:.4f}', ha='center', fontsize=10)
ax.axhline(y=cv_scores.mean(), color='#424d95', linestyle='--',
...
ax.legend()
plt.tight_layout()
plt.savefig("spaceship-titanic/rf3_cv_scores.png", dpi=150, bbox_inches='tight')
plt.show()
...
submission.to_csv("spaceship-titanic/submission_rf3.csv", index=False)
