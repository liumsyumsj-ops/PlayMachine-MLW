...
def _preprocess(train_raw, test_raw):
    ...
    # Global fallback for columns that group filling cannot solve
    df['HomePlanet'] = df['HomePlanet'].fillna('Missing')
    df['Destination'] = df['Destination'].fillna('Missing')

    # Re-split Cabin after group filling
    df[['Deck', 'CabinNum', 'Side']] = df['Cabin'].str.split('/', expand=True)
    df['CabinNum'] = pd.to_numeric(df['CabinNum'], errors='coerce')
    ...
    if 'Surname' not in final_cat_cols:
        final_cat_cols.append('Surname')
    ...
    train_missing = base_train.isna().sum()
    test_missing = base_test.isna().sum()

    train_missing = train_missing[train_missing > 0]
    test_missing = test_missing[test_missing > 0]

    if len(train_missing) > 0:
        print("\nWarning: Missing values still exist in base_train:")
        print(train_missing)

    if len(test_missing) > 0:
        print("\nWarning: Missing values still exist in base_test:")
        print(test_missing)
